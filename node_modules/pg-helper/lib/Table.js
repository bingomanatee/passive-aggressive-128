...
_.extend(Table.prototype, {
    ...
    _DPK: _.template('DELETE FROM <%= _q(table.name) %> WHERE <%= _q(pk.name) %> = <%= pk.value(record[pk.name]) %>;'),

    insert: function (client, record, returning, cb, no_clear) {
        ...
        var pk = this.pk_column();
        var self = this;

        if (cb) {
            if (!no_clear) {

                if (pk && record.hasOwnProperty(pk.name)) {
                    client.query(this._DPK({pk: pk, record: record, _q: _q, table: this}), function () {
                        self.insert(client, record, returning, cb, true);
                    });
                    return;
                }
            }
            ...
            if (!no_clear) {
                ...
                if (pk && record.hasOwnProperty(pk.name)) {
                    client.query(this._DPK({pk: pk, record: record, _q: _q, table: self}), function () {

                        client.query(self.insert_sql(record), function (err, result) {
                            if (err) {
                                deferred.reject(err);
                            } else {
                                deferred.resolve(result);
